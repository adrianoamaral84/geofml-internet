...
class SecureEditarPedidoConfirmController extends Controller
{
    ...
    public function confirmar(Request $request)
    {
        ...
        if (!in_array($pedidoAtual->status, [0], true)) {
            \Session::flash('message', [
        ...
        if (($diasAltaTemporada + $diasBaixaTemporada) !== $diasHospedagem) {
            \Session::flash('message', [
                'msg' => 'Não foi possível classificar todas as diárias do período.',
                'class' => 'danger',
            ]);
        ...
        return view('hospedagem.confirmarantigoEdit', [
            'consulta' => $consulta,
            'tipos' => $tipos,
